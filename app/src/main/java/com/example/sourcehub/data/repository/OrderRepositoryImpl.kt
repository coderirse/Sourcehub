...
class OrderRepositoryImpl(
    ...
    override suspend fun createOrder(
        userId: String, items: List<Pair<String, Int>>, couponCode: String
    ): Resource<Order> {
        return try {
            val r = orderApi.createOrder(CreateOrderRequest(
                userId, items.map { OrderItemRequest(it.first, it.second) }, couponCode
            ))
            if (r.code == 200 && r.data != null) Resource.Success(r.data.toDomain())
            else Resource.Error(r.message)
        } catch (e: Exception) {
            Resource.Error(e.message ?: "创建订单失败")
        }
    }
    ...
    override suspend fun getOrders(userId: String): Resource<List<Order>> {
        return try {
            ...
            Resource.Error(e.message ?: "加载订单失败")
        }
    }
    ...
    override suspend fun cancelOrder(orderId: String): Resource<Order> {
        return try {
            ...
            Resource.Error(e.message ?: "取消失败")
        }
    }

    private fun com.example.sourcehub.data.remote.dto.OrderResponse.toDomain(): Order {
        val orderItems = parseOrderItems(itemsJson, id)
        return Order(
            id, userId, orderItems, totalAmount, discountAmount, finalAmount,
    ...
    private fun parseOrderItems(json: String, orderId: String): List<OrderItem> {
        ...
        return try {
            ...
            trimmed.split("},").mapNotNull { part ->
                ...
                OrderItem("", orderId,
                    fields["productId"] ?: "", fields["productTitle"] ?: "",
                    fields["productCover"] ?: "",
                    fields["unitPrice"]?.toDoubleOrNull() ?: 0.0,
    ...
    private fun parseJsonObject(raw: String): Map<String, String> {
        ...
        while (i < chars.size) {
            ...
            val vs = i
            val ve = if (chars[vs] == '"') {
                val end = clean.indexOf('"', vs + 1)
                if (end == -1) chars.size else end + 1
            } else {
                var end = vs
                while (end < chars.size && chars[end] !in ",\n\r}") end++
                end
            }
